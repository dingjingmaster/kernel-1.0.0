 ...
 * 文件系统类型结构体
 * 
 * 该结构体定义了Linux内核支持的文件系统类型的基本信息和操作接口。
 * 每个文件系统驱动都需要实现一个此结构体的实例，并注册到内核的文件系统类型表中。
 */
struct file_system_type {
	/* 
	 * 读取超级块函数指针
	 * 
	 * 功能：从设备读取文件系统超级块信息，初始化文件系统实例
	 * 参数：
	 *   - struct super_block *: 指向要填充的超级块结构
	 *   - void *: 文件系统特定的数据（如挂载选项）
	 *   - int: 静默标志（1=不打印错误信息，0=打印错误信息）
	 ...
	 * 
	 * 用途：
	 * - 用户空间挂载时指定文件系统类型（如mount -t ext2）
	 * - 内核内部识别和查找文件系统类型
	 * - 调试和日志信息输出
	 * 
	 * 示例："minix", "ext2", "msdos", "nfs", "iso9660"
	 */
	char *name;
	 ...
	 * 
	 * 典型分类：
	 * 需要块设备：ext2, minix, msdos, iso9660
	 * 不需要块设备：nfs, proc, tmpfs
	 */
	int requires_dev;
};
